  ...
  const content = (
    <>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="font-bold text-base" style={{ color: "#1B3A6B" }}>{item.name}</span>
        <span className="font-semibold text-sm" style={{ color: "#7BA33E" }}>({item.dose})</span>
        {item.badge && (
      ...
      <p className="text-sm mt-2 leading-relaxed" style={{ color: "#555" }}>{item.description}</p>
    ...
    <Collapsible.Root open={open} onOpenChange={setOpen}>
      <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: "#E8ECF1", borderLeftWidth: 4, borderLeftColor: borderColor }}>
        ...
        <Collapsible.Content className="px-4 pb-4">
          <p className="text-sm leading-relaxed" style={{ color: "#555" }}>{item.description}</p>
          <div className="mt-3 h-2 rounded-full overflow-hidden" style={{ backgroundColor: "#E8ECF1" }}>
