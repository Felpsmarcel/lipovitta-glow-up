...
var conversion_summary_default = defineTool2({
  ...
  description: "Resume o desempenho do per\xEDodo: cliques, leads, compras, receita total e origens (UTM) mais fortes.",
  inputSchema: {
  ...
  handler: async ({ days }, ctx) => {
    ...
    const supabase = supabaseForUser(ctx);
    const since = new Date(Date.now() - days * 864e5).toISOString();
    const { data, error } = await supabase.from("conversion_events").select("event_name,value,cta_location,utm_source,utm_campaign").gte("created_at", since).limit(5e3);
    if (error) return { content: [{ type: "text", text: error.message }], isError: true };
    const rows = data ?? [];
    const byEvent = {};
    const bySource = {};
